...
let btnOpciones = document.querySelectorAll('.btn-opciones');
...
// Función Principal para consumir la data de la Api
const listaProductos = async () => {
    try {
        const response = await fetch('https://api.escuelajs.co/api/v1/products')
        const products = await response.json()
        // Limitar a 30 productos
        productsThirty = products.splice(0, 30);

        // Funcion para imprimir el resultado de la conversión de moneda en pantalla
        ...
        // Imprimir los productos que traemos de la Api en pantalla
        function ImprimirPantallaConProductos(productosTodos) {
        ...
        // Boton para cambiar las categorias de los productos
        opciones.addEventListener('change', (e) => {
            if (e.target.value === "Todos") {
                console.log(productsThirty);
                ImprimirPantallaConProductos(productsThirty);
                imprimirCarrito();
            }else{
...
// Imprimir en el Modal de Bootstrap los productos que agregamos al LocalStorage
...
function agregarAlCarrito(id){
    ...
    if (prodExiste) {
        ...
        const item = productsThirty.find((producto) => producto.id === id);
        item.cantidad = 1;
